...
void Sha256::update(const void* data, std::size_t length) {
    ...
    while (length > 0) {
        const std::size_t fill = 64 - buffer_length_;
        const std::size_t take = length < fill ? length : fill;
...
std::string Sha256::finish_hex() {
    ...
    for (int i = 0; i < 8; ++i) {
        length_bytes[i] = static_cast<std::uint8_t>((bit_length >> (56 - i * 8)) & 0xFF);
    }
    // Bypass update() to avoid double-counting the length bits.
    std::memcpy(buffer_ + 56, length_bytes, 8);
    process_block(buffer_);
    buffer_length_ = 0;
    ...
    for (int i = 0; i < 8; ++i) {
        for (int shift = 28; shift >= 0; shift -= 4) {
            out.push_back(hex[(state_[i] >> shift) & 0xF]);
        }
    }
    return out;
}
