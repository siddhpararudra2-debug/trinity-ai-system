...
@dataclass
class WorkflowNode:
    id: str; engine: str; operation: str; parameters: dict[str, Any]
...
def ordered(nodes: list[WorkflowNode]) -> list[WorkflowNode]:
    ...
    while remaining:
        ...
        if not ready: raise ValueError("Workflow has a missing dependency or cycle")
        for node in ready: result.append(node); del remaining[node.id]
    return result
