...
class MathEngine:
    ...
    def _extract_math(self, text: str) -> str:
        """Strip natural-language words from the beginning to avoid mangling."""
        stopwords = [
            "solve for", "solve", "calculate", "compute", "integrate", "differentiate",
            "simplify", "expand", "factor", "find", "evaluate", "what is",
            "please", "the", "of",
        ]
        cleaned = text.strip(" ?.,;:")
        for word in stopwords:
            # Strip if it appears at the start of the string
            cleaned = re.sub(rf"^{re.escape(word)}\b\s*", "", cleaned, flags=re.IGNORECASE)
        
        # also try stripping colon after "solve for x:"
    ...
    def _solve(self, expr_str: str, original: str) -> dict[str, Any]:
        ...
        x = symbols_found[0] if symbols_found else _x
    ...
    def _integrate(self, expr_str: str, original: str) -> dict[str, Any]:
        ...
        return {
            "expression": original,
            "result": f"{result} + C",
            "latex": rf"\int {latex(expr)} \, d{x} = {latex(result)} + C",
            "steps": [
    ...
    def _differentiate(self, expr_str: str, original: str) -> dict[str, Any]:
        ...
        return {
            "expression": original,
            "result": str(result),
            "latex": rf"\frac{{d}}{{d{x}}}\left({latex(expr)}\right) = {latex(result)}",
            "steps": [
