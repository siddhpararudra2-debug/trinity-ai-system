    ...
    def _extract_params(self, text: str, overrides: dict) -> dict:
        ...
        def get_val(keywords, default):
            for kw in keywords:
                ...
                m = re.search(rf"(\d+(?:\.\d+)?)\s*(?:{kw})", text, re.IGNORECASE)
                if m: return float(m.group(1))
            return default

        p = {
            "arm1": get_val(["arm1", "length1", "width"], nums[0] if len(nums) > 0 else 50.0),
            "arm2": get_val(["arm2", "length2", "height"], nums[1] if len(nums) > 1 else 50.0),
            "thick": get_val(["thick", "thickness", "t"], nums[2] if len(nums) > 2 else 5.0),
            "wbox": get_val(["w", "width", "wbox"], nums[0] if len(nums) > 0 else 80.0),
            "dep": get_val(["d", "depth", "dep", "length"], nums[1] if len(nums) > 1 else 60.0),
            "ht": get_val(["h", "height", "ht"], nums[2] if len(nums) > 2 else 40.0),
            "wall": get_val(["wall", "shell"], 3.0),
            "teeth": int(get_val(["teeth", "t", "n"], nums[0] if len(nums) > 0 else 24)),
            "mod": get_val(["mod", "module", "m"], nums[1] if len(nums) > 1 else 1.5),
            "face_w": get_val(["face", "face width"], nums[2] if len(nums) > 2 else 10.0),
            "dia": get_val(["dia", "diameter", "d"], nums[0] if len(nums) > 0 else 20.0),
            "length": get_val(["length", "len", "l"], nums[1] if len(nums) > 1 else 100.0),
            "hole_dia": get_val(["hole"], 4.0),
        }
        p.update(overrides)
        return p
...
def run(context):
    ...
    try:
        ...
        for face in body.faces:
            if abs(face.boundingBox.maxPoint.z - H*CM) < 0.1 and abs(face.boundingBox.minPoint.z - H*CM) < 0.1:
                top_face = face
                break
        if top_face:
