...
class TrinityOrchestrator:
    ...
    async def route(self, query: str, history: list = None) -> dict[str, Any]:
        ...
        try:
            return await self._dispatch(engine_id, query, history)
    ...
    async def _dispatch(self, engine_id: str, query: str, history: list = None) -> dict[str, Any]:
        if engine_id == "math":
    ...
    def _fmt_literature(self, r: dict) -> str:
        ...
        papers = r.get("papers", [])
        if any(p.get("is_fallback") for p in papers):
            lines.append("\n> **Warning:** Network error — these are simulated fallback results, not genuine arXiv papers.")
    ...
    def _fmt_vision(self, r: dict) -> str:
        if r.get("status") == "awaiting_image":
            return (
                ...
                "Upload an image via `POST /api/engines/vision/upload` and I'll extract text or LaTeX formulas. "
